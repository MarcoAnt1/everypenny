...
router.post('/', async (req: Request, res: Response) => {
    try {
        const { name, description, targetAmount, currentAmount, targetDate } = req.body;
        ...
        res.status(500).json({ error: 'Failed to create goal', details: error instanceof Error ? error.message : 'Unknown error' });
    }
});
...
router.patch('/:id/add-funds', async (req: Request<{id: string}>, res: Response) => {
    try {
        ...
        res.json({
            ...updatedGoal,
            percentage: Math.round((updatedGoal.currentAmount / updatedGoal.targetAmount) * 100),
            remaningAmount: updatedGoal.targetAmount - updatedGoal.currentAmount
        });
    } catch (error) {
        res.status(500).json({ error: 'Failed to add funds to goal', details: error instanceof Error ? error.message : 'Unknown error' });
    }
});
...
router.delete('/:id', async (req: Request<{id: string}>, res: Response) => {
    try {
        ...
        res.status(500).json({ error: 'Failed to delete goal', details: error instanceof Error ? error.message : 'Unknown error' });
    }
}); 
