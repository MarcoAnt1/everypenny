<template>
  <div class="space-y-6">
        ...
        <div>
          ...
          <div class="w-full bg-gray-100 rounded-full h-3">
            <div
              ...
              :class="
                goal.status === 'completed' ? 'bg-green-500' : 'bg-indigo-500'
              "
    ...
    <DeleteConfirmation
      ...
      :item="'Goal'"
      :item-description="deletingGoal?.name"
      @close="showDeleteConfirm = false"
      @deleted="deleteGoalConfirmed"
    />
  </div>
</template>
...
const totalSaved = computed(() =>
  goals.value.reduce((s, g) => s + Number(g.currentAmount), 0),
);
const totalNeeded = computed(() =>
  goals.value
    .filter((g) => g.status === "active")
    .reduce((s, g) => s + Number(g.remainingAmount), 0),
);
